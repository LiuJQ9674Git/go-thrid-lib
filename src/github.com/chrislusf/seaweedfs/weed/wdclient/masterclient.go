...
func (mc *MasterClient) tryAllMasters() {
	for _, master := range mc.masters {//字符串
		...
		gprcErr := withMasterClient(context.Background(), master, mc.grpcDialOption,
			func(ctx context.Context, client master_pb.SeaweedClient) error {//func匿名函数
...
//fn函数
func withMasterClient(ctx context.Context, master string, grpcDialOption grpc.DialOption,
	fn func(ctx context.Context, client master_pb.SeaweedClient) error) error {
	...
	//util.WithCachedGrpcClient
	return util.WithCachedGrpcClient(ctx,
		func(grpcConnection *grpc.ClientConn) error {
		client := master_pb.NewSeaweedClient(grpcConnection)
		return fn(ctx, client)//具体函数调用
	},
